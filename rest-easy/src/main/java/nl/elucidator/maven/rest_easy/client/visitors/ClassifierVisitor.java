 ...
 */
public class ClassifierVisitor implements ResponseVisitor {
    private List<SearchArtifact> artifacts = new ArrayList<SearchArtifact>();
    private final Classifiers classifier;

    public ClassifierVisitor(Classifiers classifier) {
    ...
    public void visit(NexusNGArtifact nexusArtifact) {
        for (ArtifactHit artifactHit : nexusArtifact.getArtifactHits()) {
            for (ArtifactLink link: artifactHit.getArtifactLinks()) {
                if ((classifier.equals(Classifiers.NULL) && link.getClassifier() == null) || (link.getClassifier().equalsIgnoreCase(classifier.toString()))) {
